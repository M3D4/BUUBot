...
class request_utils(object):
    ...
    def html_code_free(self, raw_text):
        return raw_text.replace('&nbsp;', '').replace('<font color=#FF0000>', '').replace('</font>', '')
    ...
    def lib_search_books(self, word, page):
        ...
        payload = { \
                    'solrType': 'anyword', 'libraryId': 'A', 'author': '', \
                    'publisher': '', 'lsbn': '', 'publishDate': '', \
                    'match': 'qx', 'type': 'all', 'status': '', \
                    'orderBy': '', 'pageNo': page, 'pageSize': 5, \
                    'keyword': word
                    }

        self.session.headers['Referer'] = self.get_real_domain('opac') + 'search-full-result.html'
        r = self.session.post(self.get_real_domain('opac') + 'search-full-result.json', data = payload, verify = False)
        result = r.json()
        ...
        for single_item in result['data']:
            ret_key_dict = {}
            ret_key_dict['name'] = self.html_code_free(single_item['title'])
    ...
    def lib_login(self, userId):
        ...
        self.session.headers['Referer'] = self.get_real_domain('opac') + 'index.html'
        r = self.session.post(self.get_real_domain('opac') + 'logon.json', data = payload)
    ...
    def lib_lend_book_list(self, userId):
        ...
        self.session.headers['Referer'] = self.get_real_domain('opac') + 'reader-borrowinfo.html'
        payload = {
            'pageNo': 1,
            'pageSize': 100
        }
        r = self.session.post(self.get_real_domain('opac') + 'reader-borrowinfo.json', data = payload)
    ...
    def lib_lend_book_renew(self, userId, index):
        ...
        for book in self.lib_lend_book_list(userId):
            ...
            if i == index:
                ...
                r = self.session.post(self.get_real_domain('opac') + 'xujie.json', data = payload)
